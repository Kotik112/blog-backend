...
class BlogPostControllerIntegrationTest extends SpringBootComponentTest {
  ...
  void test_createBlogPostWithImageSuccess() throws Exception {
    ...
    MockMultipartFile titlePart =
        new MockMultipartFile("title", "", "text/plain", title.getBytes(StandardCharsets.UTF_8));
    MockMultipartFile contentPart =
        new MockMultipartFile(
            "content", "", "text/plain", content.getBytes(StandardCharsets.UTF_8));
    MockMultipartFile image =
        new MockMultipartFile("image", "image.jpg", "image/jpeg", "<<jpeg data>>".getBytes());

    MvcResult mvcResult =
        mvc.perform(multipart(BASE_BLOG_POST_URL).file(titlePart).file(contentPart).file(image))
  ...
  void when_getAllBlogPosts_then_returnBlogPosts() throws Exception {
    ...
    MvcResult mvcResult1 =
        mvc.perform(get(BASE_BLOG_POST_URL + "?page=0&size=2"))
    ...
    MvcResult mvcResult2 =
        mvc.perform(get(BASE_BLOG_POST_URL + "?page=1&size=2"))
            .andExpect(status().isOk())
            .andExpect(jsonPath("$.content", hasSize(2)))
  ...
  void when_getBlogPostById_then_returnBlogPost() throws Exception {
    ...
    MockMultipartFile contentPart =
        new MockMultipartFile(
            "content", "", "text/plain", "Test content".getBytes(StandardCharsets.UTF_8));
    ...
    ResultActions resultActions =
        mvc.perform(get(BASE_BLOG_POST_URL + "/{id}", blogPostId))
            .andExpect(status().isOk())
